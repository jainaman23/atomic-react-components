...
const GetList = ({ btnStyle, renderSlideThumb }: GetListProps): Node => {
  const { activeSlideIndex, setSlideIndex, slides } = useContext(CarouselContext);
  const allSlides = [];
  ...
  return (
    <section className={classnames('carousel-wrapper', className, { legacy })} {...others}>
      <CarouselContext.Provider value={carouselState}>
        {children}
        {dots && <ListWrapper renderSlideThumb />}
...
Carousel.Slide = ({ children, id, className }: SlideProps): Node => {
  const { activeSlideIndex } = useContext(CarouselContext);

  return (
    <div className={classnames('slide', { active: id === activeSlideIndex }, className)}>
      {children}
    </div>
  );
};
